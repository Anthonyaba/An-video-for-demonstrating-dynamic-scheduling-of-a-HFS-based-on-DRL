...
class Job_encoder(nn.Module):
    def __init__(self, par):
        ...
        in_dim = self.par['job feature']
        self.GAT_layers = nn.ModuleList()
        for i in range(4):
            self.GAT_layers.append(GATLayer(in_dim, self.par['job hidden']))
            in_dim = self.par['job hidden']

    def forward(self, adjacency, feature, mask):
        # Encoding the jobs of one stage into a hidden vector using GAT layers and pooling
        output_feature = feature

        for i in range(4):  # Loop over 4 GAT layers
            # Apply GAT layer and LayerNorm
            output_feature = self.GAT_layers[i](adjacency, output_feature)
        ...
        # Pooling step:
        output = torch.mean(output_feature, dim=-2)
        return output
