...
def set_job_input(job_features, adjacency):
    ...
    for i in job_features:
        ...
        # i [jobs, nodes, feature] -> [1, jobs, nodes, features]
        tmp_i = torch.tensor(i, dtype=torch.float32).to(device).unsqueeze(0)
        #tmp_i = feature_norm_3D(tmp_i)  # Optional: Normalize the features if necessary
    ...
    for i in range(len(job_features)):
        ...
        # [job_num, nodes, nodes] -> [1, job_num, nodes, nodes]
...
def set_seq_input(action_seq_input):
    ...
    #action_seq_input = feature_norm_3D(action_seq_input, exCept_index)  # Optional: Normalize the sequence
...
def set_jm_input(job_feature_input, job_adj):
    ...
    #job_feature_input = feature_norm_3D(job_feature_input)  # Optional: Normalize the job features
